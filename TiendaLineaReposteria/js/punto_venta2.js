window.onload = function(){
  ...
  //Creamos nuestra funcion de validacion
      ...
      var patron = /^\s+/;
      var opciones = ["Chocolate","Fresa","Vainilla","Limon","Uva","Piña"];
  ...
  function showProducts(){
      if(selectElement.value=="Helado"){
          container2.textContent = "Chocolate (25.0), Vainilla (12.0), Fresa (9.98), Limon (6.0), Uva (7.5)";
      }
      else if (selectElement.value=="Torta"){
          container2.textContent = "Chocolate (14.0), Vainilla (12.0), Fresa (9.98), Limon (6.0), Uva (7.5)";
      }
  }
  ...
  Cantidad: ${producto.cantidad} chico y mas $10 precio grande <br>  Precio: ${producto.precio}  <br> Complementos: ${producto.extras}
  
  <br> Total a pagar: ${producto.getTotal()}</p>
  ...
  function Producto(id, tipo, sabor, cantidad, tamaño, complementos){
      ...
      for (var i=0; i<complementos.length;i++){
             if(complementos[i].checked == true){
                 adicional+=4.00;
                 this.extras+=complementos[i].value+",  5.00";
             }
      }
